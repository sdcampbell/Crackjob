...
hashcat -m 1000 --session crackjob -a 3 --potfile-path $(pwd)/ntpotfile --increment --increment-min 1 --increment-max 8 ${hashfile} ?a?a?a?a?a?a?a?a --show > cracked.txt
...
cat $1 | grep -v '\$' | cut -d ':' -f 4 | sed '/:$/!s/$/:/' > junk1
